#!/usr/bin/env python3
'''
esm2markdown - McAfee ESM correlation rule XML export to markdown converter - 0gxd14g
'''
...
def readConfig():
    global key_style, value_style, sort_rules, toc, imagepath  # Added global declarations
    ...
    if key:
        if typ == "list":
            if level == 1:
                ...
                lvl = "        * "
            if value == "N/A":
                output = lvl + key_style + key + key_style + "\n"
            elif value:
                output = lvl + key_style + key + key_style + " " + \
...
def generateGraph(cdata, dMatchBlocks, rid):
    ...
    if dependencies:
        ...
        for x in dMatchBlocks:
            G.add_node(x, color=dMatchBlocks[x]['shapecol'],
                       style='filled', fillcolor=dMatchBlocks[x]['shapecol'], shape='box')
        ...
        try:
            ...
            dependencies = False
            print("'dot' could not be found. Please install Graphviz.")
...
def parseXML(xmlfile):
    ...
    for rule in root.iter('rule'):
        ...
        for tags in rule.iter('tag'):
            addLine("list", 1, "Tag:", tags.text)
        for rs in cdata.iter('ruleset'):
            ...
            if not r.get('name') == "Root Rule":
                ...
                for e in r.iter():
                    ...
                    if o and v and t:
                        ...
                        addLine("list", 3, "Condition:", "'" + t + "' " + o + " '" + v + "'")
...
def convertMarkdownToDocx(mdfile):
    try:
        docxfile = os.path.splitext(mdfile)[0] + '.docx'
        check_call(['pandoc', mdfile, '-o', docxfile])
        print(f"Successfully converted {mdfile} to {docxfile}")
    except OSError as e:
        print("'pandoc' could not be found. Please install Pandoc to convert to docx.")
    except Exception as e:
        print(f"An error occurred while converting to docx: {e}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        ...
        print('Usage: python esm2markdown.py <rule xml file> <output file>')
        print('Example: python esm2markdown.py RuleExport.xml documentation.md')
    else:
        readConfig()
        parseXML(sys.argv[1])
        writeMarkdownFile(sys.argv[2])
        # Convert Markdown to DOCX
        convertMarkdownToDocx(sys.argv[2])
